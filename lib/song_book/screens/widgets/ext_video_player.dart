...
class VideoPlayerController extends ValueNotifier<VideoPlayerValue> {
  ...
  Future<void> initialize() async {
    ...
    VideoQuality quality = youtubeVideoQuality ?? VideoQuality.high3072;

    String finalYoutubeUrl = dataSource;
    if (getIdFromUrl(dataSource) != null) {
      try {
        // Map<String, String> videoUrls = {};
        String? _videoId = getIdFromUrl(dataSource);
        ...
        // log.e(manifest.muxed.length);
        if (videoUri == null) {
          finalYoutubeUrl = manifest.muxed.last.url.toString();
        } else {
          finalYoutubeUrl = videoUri.toString();
          print('finalYoutubeUrl $finalYoutubeUrl');
        }
      } catch (err) {}
    }
    ...
    void eventListener(VideoEvent event) {
      ...
      switch (event.eventType) {
        case VideoEventType.initialized:
          ...
          // log.e(event.size!.height);
          // log.e(event.size!.width);
  ...
  static String getIdFromUrl(String url, [bool trimWhitespaces = true]) {
    if (url.isNotEmpty && url.contains('yout')) {
  ...
  Future<void> dispose() async {
    if (_creatingCompleter != null) {
      ...
      if (_lifeCycleObserver != null) _lifeCycleObserver!.dispose();
    }
    _isDisposed = true;
    super.dispose();
  }
